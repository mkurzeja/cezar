...
| `GLAB_CONFIG_DIR` | The GitLab CLI's **own** variable, honoured the same way. It is how cezar finds which GitLab instances you are authenticated against — including a self-hosted one, whose hostname no table could ship — so relocating glab's config relocates that discovery too. Cezar reads only the `hosts:` keys out of that file and never its tokens. Nothing to set for a default `glab` install. |
